...
// ================= MAGIC LINK REDIRECT (Custom Scheme Fallback) =================
exports.magicRedirect = (req, res) => {
  const { token } = req.query;
  if (!token) return res.status(400).send("Token is missing.");

  // Redirect to the Flutter app's Custom Scheme
  const customScheme = `dermalyze://activate?token=${token}`;
  ...
  res.send(`
    ...
    <head>
      ...
      <title>تفعيل الحساب - Dermalyze</title>
      <style>
        body { font-family: system-ui, sans-serif; text-align: center; padding: 50px; background: #f0fdf4; color: #166534; }
        .spinner { border: 4px solid #bbf7d0; border-top: 4px solid #22c55e; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; margin: 20px auto; }
        @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
        a { display: inline-block; margin-top: 20px; padding: 10px 20px; background: #22c55e; color: white; text-decoration: none; border-radius: 5px; }
      </style>
      <script>
    ...
    <body>
      <h2>جاري تحويلك إلى تطبيق Dermalyze...</h2>
      <div class="spinner"></div>
      <p>إذا لم يتم فتح التطبيق تلقائياً، يرجى الضغط على الزر أدناه:</p>
      <a href="${customScheme}">فتح التطبيق</a>
    </body>
    </html>
  `);
};
